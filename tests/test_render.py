...
def test_pistyx_pop_is_a_server_not_a_default_route():
    out = render_pistyx_pop(
        "<priv>",
        [{"name": "laptop", "public_key": "PUBKEY", "ipv4": "10.0.1.64", "ipv6": None}],
        listen_port=47801,
        mtu=1420,
        gateway_v4="10.0.1.254",
        gateway_v6="fd00:cafe:0:1::fe",
        stack_mode="dual-stack",
    )
    assert "Address = 10.0.1.254/32" in out          # service host route; StyxSite owns the /24
    assert "ListenPort = 47801" in out
    assert "MTU = 1420" in out
    assert "PublicKey = PUBKEY" in out
    assert "AllowedIPs = 10.0.1.64/32" in out          # routed by the client's /32
...
def test_pistyx_pop_with_no_clients_has_no_peers():
    out = render_pistyx_pop(
        ...
        gateway_v4="10.0.1.254", gateway_v6="fd00:cafe:0:1::fe",
    )
...
def test_client_config_full_tunnels_to_pistyx():
    out = render_client_config(
        ...
        address_v4="10.0.1.64", address_v6="fd00:cafe:0:1::40", mtu=1420,
    )
...
def test_client_config_ipv4_only_prunes_v6():
    out = render_client_config(
        ...
        address_v4="10.0.1.64", address_v6=None, mtu=1420,
    )
...
def test_site_entrypoint_routes_pi_site_identities():
    members = [
        SiteMember(
            name="pegasus", role="init-server", site_index=1, host_suffix=1,
            ipv4="10.0.1.1", ipv6=None, public_key="PEGASUS",
        ),
        SiteMember(
            name="hydra", role="server", site_index=1, host_suffix=2,
            ipv4="10.0.1.2", ipv6=None, public_key="HYDRA",
        ),
    ]
    ...
    assert "Address = 10.0.1.1/24" in out
    assert "ListenPort = 47821" in out
    assert "PublicKey = HYDRA" in out
    assert "AllowedIPs = 10.0.1.2/32" in out
...
def test_site_remote_pi_routes_site_scope_to_entrypoint():
    members = [
        SiteMember(
            name="pegasus", role="init-server", site_index=1, host_suffix=1,
            ipv4="10.0.1.1", ipv6=None, public_key="PEGASUS", endpoint="pipegasus.duckdns.org",
        ),
        SiteMember(
            name="hydra", role="server", site_index=1, host_suffix=2,
            ipv4="10.0.1.2", ipv6=None, public_key="HYDRA",
    ...
    assert "Address = 10.0.1.2/24" in out
...
""",
    ...
    assert "Address = 10.0.2.1/24" in out
    assert "--- hydra [StyxSite1] ---" in out
    assert "Address = 10.0.1.3/24" in out


def test_pistyx_clients_parses_and_skips_incomplete():
    cfg = {"clients": [
        {"name": "good", "public_key": "K", "ipv4": "10.0.250.64"},
    ...
    assert clients[0]["host_suffix"] == 64


def test_pistyx_clients_rehomes_suffix_into_site_scope():
    cfg = {"clients": [{"name": "good", "public_key": "K", "ipv4": "10.0.250.70"}]}
