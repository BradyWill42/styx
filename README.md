...
- Documentation audit `2026-06-15 06:00 UTC`: fetched all remote branches (`main`, `MVP1`, and `MVP2`); no branch tips changed since the previous README audit after its push (`main` at `93904cc`, `MVP1` at `b55002b`, `MVP2` at `75ab2c`) before this README-only edit, so the branch-only notes below remain current.
