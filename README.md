...
- MVP3 `deploy all`, top-level `status` / `doctor`, `mesh plan` / `mesh up`, client registration, and pistyx probing/negotiation
...
| `mesh pistyx pubkey-local` | Ensure the shared pistyx keypair exists locally and print its public key |
...
| Command | Description |
|---------|-------------|
| `deploy dns plan/apply/delete` | Render, deploy, or remove the per-site DuckDNS publishers; `apply` reads `$DUCKDNS_TOKEN` |
| `deploy resolver plan/apply/delete` | Render, deploy, or remove the node-local DNS resolver + optional resolv.conf enforcer |
| `deploy reresolve plan/apply/delete` | Render, deploy, or remove the WireGuard endpoint re-resolver DaemonSet |
| `deploy all plan/apply/delete` | Render, deploy, or remove the full Styx pod-service bundle at once |
...
src/styxctl/
  ...
  ports.py            # Reserved port scan and multi-protocol port plan
  ...
  network_plan.py     # Built-in CIDRs and site-scoped IP assignment
  network_detect.py   # Public IP, DuckDNS resolve, LAN scan
  gateway.py          # Gateway SSH / k3s API ports (co-located on 47800/47801)
  ...
  cluster_dns.py      # Node-local DNS resolver + resolv.conf enforcer manifests
  reresolve.py        # WireGuard endpoint re-resolver DaemonSet + client timer
  cluster_status.py   # Top-level status / doctor workload checks
  wireguard_mesh.py   # Backbone mesh, per-site overlays, pistyx PoP, client configs
  pistyx_select.py    # Fastest-site RTT ranking and holder selection (pure)
  pistyx_repoint.py   # Persist the pistyx.current_host move into styx.yaml
  client_registry.py  # Register client peers into the styx.yaml clients: block
...
.github/workflows/
  ...
  mvp3-connectivity.yml
  pistyx-hosted-smoke.yml
  runner-smoke.yml
...
.github/scripts/
  ...
  mvp3-connectivity.py
  pistyx_hosted_smoke.py
  print_report.py
