...
- Documentation audit `2026-06-16 10:00 UTC`: fetched all remote branches (`main`, `MVP1`, and `MVP2`); no branch tips changed since the 09:00 README audit after its push (`main` at `85c304e`, `MVP1` at `b55002b`, `MVP2` at `75ab2c6`) before this README-only edit, so the branch-only notes below remain current.
