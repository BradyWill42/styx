...
- **Per-site Pi overlays** (`StyxSite<N>`) — every Pi renders one site WG config per physical WAN site, preserving the Pi's Styx host suffix across all `10.0.N.0/24` site scopes. The site entrypoint routes the other Pi identities for that site. Stage 3 of `Styx runner integration` now checks this live from every online runner; if GitHub only sees one public-IP site, the cross-site ping portion is skipped while interface and DNS checks still run.
...
1. **Watch `Styx runner integration` Stage 3** against the live five-device mesh → prove cross-site `10.0.N.x` reachability and DNS from every online runner.
...
- **No local Python on the dev machine** → CI is the only verifier. Per-push CI now covers live WireGuard mesh behavior; k3s install and pod runtime behavior (`deploy dns apply`, `status`/`doctor`) are **e2e-only**.
