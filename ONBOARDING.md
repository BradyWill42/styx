...
- **Client registration + pistyx probe** — `client config --register` persists roadwarrior peers into `clients:` and renders matching site-scoped addresses; `mesh pistyx probe <client-ip>` ranks site leaders by RTT and recommends the current best pistyx holder.

## 2. Designed but deferred (the rest of the overlay)

The **backbone** mesh, per-site Pi overlays, client registration, and pistyx probe are built.
Still **designed, not built**: `styx`-as-movable-hub, leader-to-styx uplinks beyond the current
routed site entrypoints, and the automatic `pistyx` quickest-site repoint loop. These are only
meaningfully testable once **pithor** is a real *remote* second site.
...
4. **Automatic `pistyx` quickest-site loop** — `mesh pistyx probe <client-ip>` recommends the fastest holder today; the deferred part is automatically editing `pistyx.current_host`, rerunning `mesh up`, and republishing DNS.
...
- **Deferred overlay work:** leader-to-styx uplinks beyond the current routed entrypoints, movable `styx` hub behavior, and the automatic `pistyx` quickest-site repoint loop are still planned.
