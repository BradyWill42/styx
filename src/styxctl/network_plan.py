...
# The egress /24 (and /64): .1 is pistyx — the SHARED PoP gateway address every leader binds
# (reserved as PISTYX_IPV4/6 above) — and roadwarrior clients take .2 upward. Each site's leader
# runs the pistyx PoP at .1 and local-NATs the client range out its OWN WAN; styx never backhauls.
ROADWARRIOR_CLIENT_OFFSET = 2


def roadwarrior_ipv4_for_index(index: int) -> str:
    """Roadwarrior client IPv4 for the given client slot (+2 keeps .0 network and .1 pistyx reserved)."""
